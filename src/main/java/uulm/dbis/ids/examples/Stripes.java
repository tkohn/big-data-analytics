package uulm.dbis.ids.examples;
...
public class Stripes extends Configured implements Tool {
	...
	public static class Reduce extends Reducer<Text, MapWritable, Text, MapWritable> {
		...
		public void reduce(Text key, Iterable<MapWritable> values, Context context)
			...
			for (MapWritable localStripe : values) {
				Iterator<Entry<Writable, Writable>> entries = localStripe.entrySet().iterator();

				while (entries.hasNext()) {
					Entry<Writable, Writable> thisEntry = entries.next();
	...
	public int run(String[] args) throws Exception {
		Job job = Job.getInstance(new Configuration(), "stripes");
	...
	public static void main(String[] args) throws Exception {
		int ret = ToolRunner.run(new Configuration(), new Stripes(), args);
		System.exit(ret);
	}
}